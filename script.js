const getScamReport = (text) => {
  ...
  // Category D: Malicious/Phishing/Shortened Links (UPDATED)
  ...
  if (suspiciousUrlPatterns.some((pattern) => textClean.includes(pattern)))
    scamScore += 100;
  ...
  // Category G: Platform Spoofing & Broken English (NEW)
  const platformSpoofing = [
    "oder",
    "togeter",
    "agre",
    "fiverr",
  ...
  if (platformSpoofing.some((word) => textClean.includes(word)))
    scamScore += 40;
